...
'''
    ...
    h_t是当前时刻的隐藏状态, x_t是当前输入, h_{t-1}是上一时刻的隐藏状态, U和 W是共享的权重矩阵
'''
import numpy as np
import torch
...
def manual_input(x_np, U_np, W_np):
    B, T, _ = x_np.shape
...
def main():
    ...
    # 使用可学习参数 U, W（无偏置）
    torch.manual_seed(7)
    U = torch.randn(E, H)  # (128, 3)
    W = torch.randn(H, H)  # (3, 3)
    ...
    print("--- 手写 RNN (NumPy) ---")
    out_manual_np, hT_manual_np = manual_input(x_np, U_np, W_np)
